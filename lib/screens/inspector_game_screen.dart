...
class _InspectorGameScreenState extends State<InspectorGameScreen>
  ...
  void _validate() {
    ...
    if (wrongPlaced.isNotEmpty || missingCorrect.isNotEmpty) {
      for (final chip in wrongPlaced) {
        _mistakeCount++;
        _addTermLine(
          TerminalLine(TerminalLineType.error, '> ERROR: ${chip.errorMessage}'),
        );
      }
  ...
  int _computeXP(int stars) =>
      50 + (stars - 1) * 15 + (widget.level.isBoss ? 60 : 0);
  ...
  void _showResultDialog({bool timedOut = false, bool compileFailed = false}) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Text(
          timedOut ? '⏱ Time\'s Up!' : '❌ Compile Error',
  ...
  Widget _buildHeader() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border(
          bottom: BorderSide(color: zone.accentColor.withOpacity(.25)),
        ),
      ),
      child: Row(
        children: [
  ...
  Widget _buildObjectiveBanner() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: zone.accentColor.withOpacity(.05),
        border: Border(
          bottom: BorderSide(color: zone.accentColor.withOpacity(.12)),
        ),
      ),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
              ...
              builder: (ctx, constraints) {
                return Stack(
                  ...
                  children: [
                    ...
                    Positioned(
                      bottom: constraints.maxHeight * 0.42,
                      left: 0,
                      right: 0,
                      child: Container(
                        height: 1,
        ...
        children: [
          ...
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...widget.level.existingComponents.map(
                    (c) => _ComponentTile(
                      name: c.name,
                      icon: c.icon,
                  ...
                  ...widget.level.scriptBank
                      ...
                      .map((c) {
                        ...
                        return _ComponentTile(
                          name: c.label,
                          icon: '⚙',
                          accentColor: correct ? c.dotColor : Colors.red,
                          fields: correct ? c.addFields : [],
                          locked: false,
                          isError: !correct,
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(5),
                          border: Border.all(
                            color: chip.dotColor.withOpacity(.2),
                          ),
                        ),
                        child: Text(
                          _codeSnippet(chip.id),
                          style: const TextStyle(
                            fontFamily: 'Courier New',
                            fontSize: 9,
                            color: Color(0xFF7DC890),
                            height: 1.6,
                          ),
                        ),
                      ),
  ...
  Widget _buildBottomBar(bool canRun, bool hasWrong) {
    ...
    final placed = _placedChips.keys
        ...
        children: [
          AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: activated
                    ? const Color(0xFF00E5FF).withOpacity(.55)
                    : Colors.white.withOpacity(.06),
    ...
    if (floatCtrl != null) {
      child = AnimatedBuilder(
        ...
        builder: (_, c) => Transform.translate(
          offset: Offset(0, -5 * floatCtrl!.value),
          child: c,
        ),
        child: child,
      );
    ...
    if (pulseCtrl != null) {
      child = AnimatedBuilder(
        ...
        builder: (_, c) => Transform.scale(
          scale: 1.0 + 0.14 * sin(pulseCtrl!.value * pi),
          child: c,
        ),
        child: child,
      );
  ...
  Widget build(BuildContext context) {
    final bdr = widget.isError
        ...
        : (widget.locked
              ...
              : widget.accentColor.withOpacity(.28));
    return Container(
                ...
                children: [
                  ...
                  if (widget.isError)
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: const Color(0xFF00C853).withOpacity(.1),
                        borderRadius: BorderRadius.circular(2),
                        border: Border.all(
                          color: const Color(0xFF00C853).withOpacity(.28),
                        ),
                      ),
                      child: const Text(
                        '● on',
...
class _ScriptBankChipState extends State<_ScriptBankChip>
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      onTapUp: (_) {
        _press.reverse();
        if (!widget.placed) widget.onTap?.call();
      },
      onTapCancel: () => _press.reverse(),
      child: AnimatedBuilder(
        animation: _press,
        builder: (_, child) =>
            Transform.scale(scale: 1.0 - 0.06 * _press.value, child: child),
            ...
            child: Row(
              ...
              children: [
                Container(
                  ...
                  height: 5,
                  decoration: const BoxDecoration(
                    // Same neutral grey dot for ALL chips
...
class _GridFloorPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i <= cols; i++) {
      ...
      paint.color = const Color(0xFF0050A0).withOpacity(.12);
      canvas.drawLine(
        Offset(x, 0),
        Offset(size.width * .5 + (x - size.width * .5) * .06, size.height),
        paint,
      );
    }
    ...
    return Dialog(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: const Color(0xFF00E5FF).withOpacity(.28),
          ),
        ),
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border(
                  bottom: BorderSide(
                    color: const Color(0xFF00E5FF).withOpacity(.14),
                  ),
                ),
              ),
              child: Row(
                  ...
                  const Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'Simulate real Unity development — no code required',
                          style: TextStyle(color: Colors.white38, fontSize: 10),
                        ),
                      ],
                    ),
                ...
                child: Column(
                  children: [
                    _bullet(
                      '🎯',
                      'Your job',
                      'Each level gives you a plain-English challenge — "make the screen show the real world", "let players grab objects". Add the right Unity scripts to the GameObject in the Inspector to make it work.',
                    ),
                    const SizedBox(height: 10),
                    _bullet(
                      '📋',
                      'Script bank',
                      'The bank shows real Unity script names. Some are correct. Some are convincing fakes. Tap any chip to add it to the Inspector — you\'ll only know if it\'s right when you Run the scene.',
                    ),
                    const SizedBox(height: 10),
                    _bullet(
                      '▶',
                      'Run Scene',
                      'When ready, tap "Run Scene". The Console shows what worked and what failed. Wrong scripts show ERR in the Inspector — tap × to remove them and try again.',
                    ),
                    const SizedBox(height: 10),
                    _bullet(
                      '⚙',
                      'Learn',
                      'Pass a level and tap "See How It Works" to read the real C# code behind each script you placed. You\'re not just guessing — you\'re learning.',
                    ),
